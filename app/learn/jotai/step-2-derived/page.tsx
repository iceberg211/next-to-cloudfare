...
const parityAtom = selectAtom(countAtom, (c) => (c % 2 === 0 ? '偶数' : '奇数'));

export default function Step2Derived() {
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-semibold">Step 2 · 派生与精确订阅</h2>
      <p className="text-zinc-600">使用 selectAtom 从基础原子投影出派生值，仅在依赖变化时重算。</p>
...
function Control() {
  ...
  return (
    <section className="rounded-lg border bg-white p-4 space-y-3">
      ...
      <p className="text-xs text-zinc-500">左侧更新 countAtom，右侧派生面板仅在需要时更新。</p>
    </section>
  );
}
...
function DerivedPanel() {
  ...
  return (
    <section className="rounded-lg border bg-white p-4 space-y-2">
      ...
      <div>×2：{double}</div>
      <div>奇偶：{parity}</div>
      <p className="text-xs text-zinc-500">selectAtom 只订阅 countAtom 的投影值，避免无关渲染。</p>
    </section>
  );
}
