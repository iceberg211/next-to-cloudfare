...
export default function Step4Async() {
  return (
    <div className="space-y-6">
      ...
      <p className="text-zinc-600">原子可直接返回 Promise。结合 Suspense 与错误边界表现加载/错误与重试。</p>
      <ErrorBoundary>
        <Suspense fallback={<div className="text-sm text-zinc-500">加载中...</div>}>
          <SuggestionList />
        </Suspense>
      </ErrorBoundary>
      <p className="text-xs text-zinc-500">刷新页面即可看到重新加载；也可扩展按钮触发刷新（set 同一异步 atom）。</p>
    </div>
  );
}
...
class ErrorBoundary extends React.Component<{ children: React.ReactNode }, { error?: Error }> {
  ...
  render() {
    if (this.state.error) {
      return <div className="text-sm text-red-600">{this.state.error.message}</div>;
    }
